...
open class HashiCorpVaultConnection() : ProjectFeature() {
    ...
    sealed class AuthMethod(value: String? = null): CompoundParam<AuthMethod>(value) {
        ...
        class GcpIam() : AuthMethod("gcp-iam") {
            ...
            override fun validate(consumer: ErrorConsumer) {
                if (gcpVaultRole == null && !hasParam("gcp-role")) {
                    consumer.consumePropertyError("authMethod.gcpVaultRole", "mandatory 'authMethod.gcpVaultRole' property is not specified")
                }
            }
        }
    }
