 ...
 *
 * **Example.**
 * Defines a Perforce VCS root for a Perforce stream with an option to automatically remove agent workspace after build finish.
 * ```
 * object StreamBasedRootWithSubStreams : PerforceVcsRoot({
 *   name = "Stream-based root"
 *   port = "perforce:1666"
 *
 *   mode = stream {
 *     streamName = "//streams/project/mainline"
...
open class PerforceVcsRoot : VcsRoot {
    ...
    /**
     * Workspaces created by bare-metal and cloud agents will be removed after a build finishes. This behavior forces a clean checkout for each new build.
     */
    var deleteCreatedWorkspaces by stringParameter("delete-agent-workspaces")
